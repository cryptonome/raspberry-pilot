...
from common.rpparams import Params, put_nonblocking
...
class Transcoderd():
  ...
  def dump_sock(self, sock, wait_for_one=False):
    if wait_for_one:
      sock.recv_one()
    while 1:
      try:
  ...
  def tri_blend(self, l_prob, r_prob, tri_value, minimize=False, minimize2=False):
  ...
  def update_calibration(self, calibration, inputs, cal_col, cs):
  ...
  def process(self,sm=None, pm=None, can_sock=None):
    INPUTS = 78
    OUTPUTS = 9
    MODEL_VERSION = 'F'
    MODEL_NAME = ''
    ...
    self.pm = pm
    if self.pm is None:
      self.pm = messaging.PubMaster(['pathPlan'])
    ...
    gernPath = messaging.PubMaster(['pathPlan'])
    carState = messaging.SubMaster(['carState'])
    ...
    gernPath.send('pathPlan' ,path_send.to_bytes())
    path_send = log.Event.new_message()
    path_send.init('pathPlan')
    ...
    lateral_adjust = 0
    frame = 0
    Transcoderd.dump_sock(carState, True)

    calibration_items = [['angle_steers','lateral_accelleration','yaw_rate_can','angle_steers2','lateral_accelleration2',
                          'yaw_rate_can2'],['far_left_1','far_left_7','far_left_9','far_right_1','far_right_7','far_right_9',
                          'left_1','left_7','left_9','right_1','right_7','right_9']]
    all_items = [['v_ego','angle_steers','lateral_accelleration','angle_rate', 'angle_rate_eps', 'yaw_rate_can','v_ego',
                  'long_accel', 'lane_width','angle_steers2','lateral_accelleration2','yaw_rate_can2'],
                 ['l_blinker','r_blinker',
    ...
    while True:
      ...
      for _cs in car.CarParams.from_bytes(Params().get("CarParams", block=True)):
...
def main(sm=None, pm=None, logcan=None):
  transcoderd = Transcoderd(sm, pm, logcan)
  transcoderd.process()
