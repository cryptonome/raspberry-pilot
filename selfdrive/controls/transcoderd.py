...
while 1:
  ...
  if cs.vEgo > 10 and abs(cs.steeringAngle - calibration[0]) <= 3 and abs(cs.steeringRate) < 3 and l_prob > 0 and r_prob > 0:
    update_calibration(calibration, np.concatenate((vehicle_array[-1], camera_input), axis=0), cal_col, cs)
    ...
    for i in range(len(cal_col)):
      if cal_factor[i] > 0:
        ...
        if i == 0: 
          vehicle_array[-1,:,cal_col[i]] /= angle_factor
          vehicle_array[-1,:,8] -= calibration[0]
          vehicle_array[-1,:,8] /= angle_factor
